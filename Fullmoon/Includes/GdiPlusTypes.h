...
public:
   ...
   /*添加于2017-9-14
   修复BUG 修改为严格弱排序  2017-9-15*/
   bool operator<(IN const Point& point) const{
	   //return X < point.X && Y < point.Y;
	   return Y == point.Y ? 
		   X < point.X:
		   Y < point.Y;
   ...
   BOOL Equals(IN const Point& point)const{
       return (X == point.X) && (Y == point.Y);
   }
...
public:
    ...
    // Determine if the specified rect intersects with the
    // current rect object.
	/*确定指定的矩形与当前的rect对象是否相交。这tm是有问题的*/
    BOOL IntersectsWith(IN const Rect & rect) const
    {
        return (GetLeft() < rect.GetRight() &&
                GetTop() < rect.GetTop() &&//这里该是rect.bottom
                GetRight() > rect.GetLeft() &&
                GetBottom() > rect.GetTop());
    }
	/*矩形a与b相交储存于c  返回是否相交*/
