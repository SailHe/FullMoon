...
public:
	DisplayArea(Sprite const &mapBody, size_t bioCount)
		...
		, ecoregionWindow(WindowSprite(_T("xx生态")))
	...
	int w = RenderManager::cameraSprite.getLocation().X + 500, h = w;// 缓冲区图片尺寸
	int clipX = 0;// 屏幕分割点，用于在屏幕上进行选区，取值为(0,w]
	int drawX = 0;// 屏幕绘制点，用于图片缓冲区和屏幕对齐，取值为(-w,0]
	int oldBufClipX = RenderManager::cameraSprite.getLocation().X;// 上一次缓冲区分割点，和屏幕分割点有一定的关系，取值为[0,w)
	int bufClipX = oldBufClipX;// 缓冲区分割点，和屏幕分割点有一定的关系，取值为[0,w)
	int bufDrawX = 0;// 缓冲区绘制点，用于在缓冲区上绘制地图信息，取值为[0,w)，注意此数为离散值，i*cellWidth

	//Image *imageBuffer = nullptr;// 缓冲区
	//Graphics *bg;// 缓冲区的画笔
	//Engine eg;
